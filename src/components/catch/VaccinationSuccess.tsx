...
import { User, Syringe, ArrowRight, MapPin } from 'lucide-react';
...
export const VaccinationSuccess: React.FC<VaccinationSuccessProps> = ({
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen px-6 text-center bg-white">
      ...
      >
        <Syringe className="w-12 h-12 text-orange-600" />
      </motion.div>
      ...
      >
        <div className="flex gap-3">
          <StatMini icon={<Syringe className="w-4 h-4" />} label={vaccineType} active />
          <StatMini icon={<MapPin className="w-4 h-4" />} label="Location locked" active />
        </div>

        <button
          onClick={onViewProfile}
          className="w-full p-5 bg-gray-50 border border-border rounded-[24px] flex items-center justify-between group hover:bg-accent/5 hover:border-accent/20 transition-all"
        >
          <div className="flex items-center gap-4 text-left">
            <div className="w-12 h-12 bg-white rounded-[16px] shadow-sm flex items-center justify-center text-accent">
              <User className="w-6 h-6" />
            </div>
            <div>
              <span className="text-[15px] font-bold text-dark block">View Profile</span>
              <span className="text-[12px] text-muted font-medium">Check history and booster dates</span>
            </div>
          </div>
      ...
      >
        <Button
          ...
          className="w-full h-[64px] bg-accent hover:bg-accent/90 text-white rounded-[22px] font-extrabold text-[17px] shadow-amber-glow"
        >
          Vaccinate Another Dog
        </Button>
        <Button
          variant="outline"
          onClick={onClose}
...
const StatMini = ({ icon, label, active }: { icon: React.ReactNode, label: string, active: boolean }) => (
  <div className={cn(
    ...
    active ? "bg-orange-50 border-orange-100 text-orange-600" : "bg-gray-50 border-gray-100 text-muted"
  )}>
    {icon}
    <span>{label}</span>
  </div>
